 ...
 * 
 * This script populates Firebase and Cloudinary with test data for development.
 * 
 * Usage:
 * 1. Make sure your .env file has Firebase and Cloudinary credentials
 * 2. Run: npx tsx scripts/seedTestData.ts
 * 
 * Or use the Node.js version: node scripts/seedTestData.js
 */
...
async function seedProducts() {
  ...
  for (const product of testProducts) {
    try {
      // For now, using direct image URLs (Unsplash)
      // In production, you would upload images to Cloudinary first
      const productData = {
        ...product,
        images: product.images, // Using direct URLs for now
...
async function uploadImageToCloudinaryFromUrl(imageUrl: string, folder: string = 'stock'): Promise<string> {
  try {
...
async function seedProductsWithCloudinary() {
  console.log('🌱 Seeding products with Cloudinary uploads...');
  console.log('⚠️  This will upload images to Cloudinary. Make sure your upload preset is configured.\n');
  ...
  for (const product of testProducts) {
    try {
      ...
      for (const imageUrl of product.images) {
        try {
          const cloudinaryUrl = await uploadImageToCloudinaryFromUrl(imageUrl, 'stock');
          uploadedImages.push(cloudinaryUrl);
          console.log(`  📤 Uploaded image: ${cloudinaryUrl.substring(0, 50)}...`);
        } catch (error) {
          console.warn(`  ⚠️  Failed to upload image, using original URL`);
  ...
  console.log('✅ Products seeding with Cloudinary complete!\n');
}
...
async function main() {
  ...
  try {
    // Seed products (with direct image URLs - faster)
    await seedProducts();
    ...
    console.log('2. If you want to upload images to Cloudinary, run: seedProductsWithCloudinary()');
    console.log('3. Test the app by browsing products and placing orders');
  } catch (error: any) {
...
export { seedProducts, seedCoupons, seedProductsWithCloudinary, testProducts, testCoupons };
